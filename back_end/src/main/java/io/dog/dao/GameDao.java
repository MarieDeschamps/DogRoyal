...
public class GameDao {
	...
	public void updateFree(int game_id,int player,boolean free) {
		String jpql = "UPDATE GameDB g SET g.free=:free WHERE g.game_id=:game_id AND g.player=:player";
		em.createQuery(jpql).setParameter("free", free).setParameter("game_id", game_id).setParameter("player", player).executeUpdate();
	}
	...
	public List<GameDB> getGames() {
		String jpql = "SELECT g FROM GameDB g";
